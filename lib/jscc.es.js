...
var DIRECTIVES = /^[ \t\f\v]*(.*?)#(if|ifn?set|el(?:if|se)|endif|set|unset|error)(?:(?=[ \t])(.*)|\/\/.*)?$/gm;
...
Parser.prototype = {
  ...
  parse: function parse (match) {         //eslint-disable-line complexity
    ...
    var key  = match[2];
    var expr = self._normalize(key, match[3]);
   ...
   */
  matchPrefixes: function matchPrefixes (match) {
    return this.options.prefixes.some(function (prefixe) {
      var result;
